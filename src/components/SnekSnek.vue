...
const score = ref(0);
const highScore = ref(0); // Nouveau state pour le record
...
const getGroundHeightAt = (x: number, y: number) => {
  if (isPlatform(x, y)) return platformHeight;
...
const setGameOver = () => {
  ...
  updateHighScore(); // Sauvegarde le score si c'est un record
...
const spawnFood = () => {
  while (true) {
    ...
    const onSnake = snake.value.some((s) => s.x === x && s.y === y && s.height < 0.5);
    if (!isJumper && !onSnake) {
      food.value = { x, y };
      return;
    }
...
const advanceOneStep = () => {
  ...
  if (Math.abs(nextH) < 0.1 && isMainBoard(nextX, nextY)) {
    const isJumper = (nextX===0&&nextY===0)||(nextX===19&&nextY===0)||(nextX===0&&nextY===19)||(nextX===19&&nextY===19);
    if (isJumper) {
      playSound('jump_up');
  ...
  if (Math.abs(nextH) < 0.1 && nextX === food.value.x && nextY === food.value.y) {
    score.value += 10;
    spawnFood();
    playSound('laser');
  } else {
...
<template>
  <div class="game-wrapper">
    ...
    <TresCanvas window-size clear-color="#111" shadows>
      ...
      <TresMesh :position="[food.x+0.5,0.45,food.y+0.5]" cast-shadow>
...
.high-score {
  font-size: 1.1em;
  color: #fb0; /* Couleur dorée pour le record */
  font-weight: bold;
}
.status {
  font-weight: bold;
